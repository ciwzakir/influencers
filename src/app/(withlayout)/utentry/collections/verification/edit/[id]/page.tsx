...
const VerifyCollections = () => {
  ...
  const role = userInfo?.user_role ?? "utentry";
  ...
  useEffect(() => {
    if (isSuccess) {
      message.success("Collection successfully verified.");
      router.push(`/${role}/collections/paid`);
      refetch();
    }
  ...
  const onSubmit: SubmitHandler<Inputs> = async (values) => {
    ...
    formData.append("action", "confirm_verification");
      ...
      >
        <div style={{ textAlign: "center", marginBottom: "24px" }}>
          <Title level={3} style={{ color: "#1890ff" }}>
            You may change data and verify
          </Title>
        </div>
              ...
              <Col xs={24} md={14} lg={16} xl={18}>
                <Row gutter={[16, 16]}>
                  ...
                  <Col xs={24} style={{ textAlign: "right" }}>
                    ...
                    >
                      {isLoading ? "Updating..." : "Confirm Verification"}
                    </Button>
                  </Col>
                </Row>
              </Col>
