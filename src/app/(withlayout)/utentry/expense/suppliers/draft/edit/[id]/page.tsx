...
const EditDraftBill = ({ params }: { params: { id: string } }) => {
  const { id } = params;
  const { data: getSingleData, isLoading, refetch } = useGetSingleBillQuery(id);
  ...
  const onSubmit = async (formData: FormValues) => {
    try {
      ...
      if (role) {
        router.push(`/${role}/expense/suppliers/sent-backs`);
        refetch();
      }
    } catch (error) {
